...
export function* editNotepadById(
    ...
    try {
        ...
        if (notepadName) {
            ...
            yield call(GithubApi.notepads.update, {id, files: {[`${title}`]: {content: description,},},});
        }
    } catch (e) {
        console.error(e);
    }
...
export function* addNotepad(action: PayloadAction<{ notepad: INotepad; cb: () => void }>) {
    const {payload: {notepad, cb},} = action;
    const files: INotepadFile = {};

    notepad.notes.forEach((note) => {
        files[note.title] = {content: note.description,};
    });
    try {
        //@ts-ignore
        const data: INotepad = yield call(GithubApi.notepads.create, {description: notepad.title, files,});
...
export function* removeNoteById(action: PayloadAction<{ data: INote; id: string }>) {
    ...
    try {
        yield call(GithubApi.notepads.update, {id, files: {[`${data.title}`]: {content: "",},},});
